...
export function CreateCompetitionForm({
  ...
  const { data: venuesRaw, isLoading: venuesLoading } = useVenues();
  ...
  if (!user) {
    return (
      <AuthenticationGuard message="You need to be logged in to create a competition." />
    );
  }
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
            ...
            >
              {createCompetition ? "Creating..." : "Create Competition"}
            </button>
          </div>
        </form>
      </div>
