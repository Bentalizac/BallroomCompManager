...
export const userRouter: any = router({
  ...
  getMyRegistrations: authedProcedure.query(async ({ ctx }) => {
    ...
    const { data: registrations, error } =
      await UserDAL.getUserEventRegistrations(
  ...
  registerForComp: authedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const { data: existingRegs, error: fetchError } =
        await UserDAL.checkCompetitionRegistration(
          supabase,
          ctx.userId,
          input.competitionId,
        );
      ...
      if (fetchError) {
        if (process.env.NODE_ENV === "development")
          console.error("Error checking existing registrations:", fetchError);
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
      ...
      const { data: newParticipants, error: participantError } =
        await UserDAL.createCompetitionParticipants(
          supabase,
          ctx.userId,
          input.competitionId,
          input.roles as Array<
            "spectator" | "competitor" | "organizer" | "judge"
          >,
        );
  ...
  getUserRoleInCompetition: authedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const { data: adminCheck, error: adminError } =
        await UserDAL.checkUserIsCompetitionAdmin(
          supabase,
          input.competitionId,
          ctx.userId,
        );
      ...
      const { data: participantRoles, error: participantError } =
        await UserDAL.getUserParticipantRoles(
          supabase,
          input.competitionId,
          ctx.userId,
        );
      ...
      if (
        ...
        try {
          const { error: insertError } =
            await UserDAL.createOrganizerParticipant(
              supabase,
              input.competitionId,
              ctx.userId,
            );
          ...
          if (insertError) {
            if (process.env.NODE_ENV === "development") {
              console.error(
                "Error creating organizer participant record:",
                insertError,
              );
            }
          } else {
            ...
            if (updatedRoles) {
              participantRoles?.push(
                ...updatedRoles.filter(
                  (r) => !participantRoles.some((p) => p.id === r.id),
                ),
              );
            }
          }
      ...
      return {
        ...
        hasAccess:
          !!isAdmin || (participantRoles && participantRoles.length > 0),
      };
    }),
